...
- **Comments acceptance tests**: Enabled all 7 acceptance tests in `tests/acceptance/comments.ts`. Fixed `CommentForm` placeholder ("Leave a comment..."), button label ("Post Comment"), 202/pending-moderation handling, and rate-limit error message passthrough. Fixed `ReplyForm` submit button label ("Post Reply"). Replaced `window.confirm` in `CommentItem` delete flow with `AlertDialog` (`role="alertdialog"`). Added `aria-label="Delete comment"` to delete button, `.comment-item` CSS class, `.comment-timestamp` element, `.badge` class on author badge, `data-comment-id` attribute, `data-testid="comments-list"` on `CommentList`, and thread-reply label "Reply to @username". Extended `formatTimestamp` to always return relative text (weeks/months/years ago) instead of falling back to `toLocaleDateString`. Added optimistic update to `usePostComment` so submitted text appears immediately. Extended `commentsApi.postComment` return type to carry `status`/`message` fields for 202 responses. Added Playwright `globalSetup` to seed the backend before acceptance tests run. Updated backend seed to publish `test-post` and add seeded comments with a reply for acceptance test data. Updated e2e delete tests (4.2, 4.4) to interact with `AlertDialog` instead of `window.confirm`.
