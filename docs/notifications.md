...
The token replaces authentication by proving the caller possesses the shared secret (SECRET_KEY).
...
```bash
# Token generated by: UnsubscribeToken.generate(user_id, email) using SECRET_KEY
...
The application secret key used to generate and verify HMAC tokens in unsubscribe links.
Tokens are generated as: `hmac_sha256("{user_id}:{email}:{YYYYMM}", SECRET_KEY)` — time-bucketed
to expire after approximately one month. Both the current and previous month buckets are accepted
during verification to accommodate tokens generated near a month boundary.

```bash
export SECRET_KEY="your_secret_key_here_min_32_chars"
```
...
- Never commit: Store in environment variables or secrets management only
- Rotate carefully: Changing this value immediately invalidates all existing unsubscribe tokens
- Note: Earlier documentation referred to a separate `UNSUBSCRIBE_SECRET` variable; the implementation
  uses `SECRET_KEY` throughout
...
- Notifications in `failed_permanently` state (max retries exhausted, manual intervention needed)
...
message = f"{user_id}:{email.lower()}:{bucket}"
