...
def unsubscribe_all() -> tuple[Response, int]:
    ...
    proving the caller possesses the shared secret (SECRET_KEY) used to
    generate the link. The user is looked up first to obtain their email,
    which is required for token verification.

    Args (query params):
        user_id: Primary key of the user to unsubscribe.
        token: HMAC-SHA256 hex digest generated by UnsubscribeToken.generate.

    Returns:
        Tuple of (JSON success message, 200).

    Raises:
        400: Missing parameters, user not found, or invalid/tampered token.
    """
    ...
    if not UnsubscribeToken.verify(token, user_id, user.email):
