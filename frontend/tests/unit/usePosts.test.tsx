...
describe("usePosts hooks", () => {
	...
	describe("useDraft", () => {
		...
		it("should return loading state initially", () => {
			vi.mocked(postsApi.getDraft).mockImplementation(
				() => new Promise(() => {}),
			);
	...
	describe("useMyPosts", () => {
		...
		it("should call postsApi.listMyPosts with default params", async () => {
			...
			expect(postsApi.listMyPosts).toHaveBeenCalledWith(
				undefined,
				1,
				20,
				"mock-token",
	...
	describe("useCreateDraft", () => {
		...
		it("should call postsApi.createDraft with correct parameters", async () => {
			...
			expect(postsApi.createDraft).toHaveBeenCalledWith(
				"new-post",
				"New Post",
				"mock-token",
			);
		...
		it("should be in loading state during mutation", async () => {
			vi.mocked(postsApi.createDraft).mockImplementation(
				() => new Promise(() => {}),
			);
	...
	describe("useSaveDraft", () => {
		...
		it("should call postsApi.saveDraft with correct parameters", async () => {
			...
			expect(postsApi.saveDraft).toHaveBeenCalledWith(
				"test-post",
				content,
				"mock-token",
			);
		...
		it("should perform optimistic update", async () => {
			...
			const optimisticData = queryClient.getQueryData<PostResponse>([
				"draft",
				"test-post",
			]);
	...
	describe("usePublishPost", () => {
		...
		it("should call postsApi.publishPost with slug", async () => {
			...
			expect(postsApi.publishPost).toHaveBeenCalledWith(
				"test-post",
				"mock-token",
			);
		});
	...
	describe("useUnpublishPost", () => {
		...
		it("should call postsApi.unpublishPost with slug", async () => {
			vi.mocked(postsApi.unpublishPost).mockResolvedValueOnce(
				mockUnpublishedPost,
			);
			...
			expect(postsApi.unpublishPost).toHaveBeenCalledWith(
				"test-post",
				"mock-token",
			);
		});
	...
	describe("useDeleteDraft", () => {
		it("should call postsApi.deleteDraft with slug", async () => {
			...
			expect(postsApi.deleteDraft).toHaveBeenCalledWith(
				"test-post",
				"mock-token",
			);
		});
