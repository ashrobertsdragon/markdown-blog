...
describe("AuthProvider", () => {
	...
	describe("Context Value Provision", () => {
		it("should provide all four context properties", () => {
			const mockUser = {
				id: "user_123",
				firstName: "John",
			...
			const TestComponent = () => {
				...
				return (
					<div>
						...
						{isLoaded && !user && (
							<div data-testid="not-loaded">Not signed in</div>
						)}
					</div>
				);
			};
		...
		it("should handle signed in state correctly", () => {
			...
			const TestComponent = () => {
				...
				return (
					<div>
						{isSignedIn && (
							<div data-testid="signed-in">Welcome, {user?.firstName}!</div>
						)}
					</div>
				);
			};
			...
			expect(screen.getByTestId("signed-in")).toHaveTextContent(
				"Welcome, SignedIn!",
			);
		});
		...
		it("should handle signed out state correctly", () => {
			...
			const TestComponent = () => {
				...
				return (
					<div>
						{isLoaded && !isSignedIn && (
							<div data-testid="signed-out">Please sign in</div>
						)}
					</div>
				);
			};
			...
			expect(screen.getByTestId("signed-out")).toHaveTextContent(
				"Please sign in",
			);
		});
